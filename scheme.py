...
class Frame:
    ...
    def __init__(self, parent):
        ...
        self.bindings = {}      #dictionary that maps Scheme symbol keys to Scheme values
        ...
        def recursor(formals, vals):
            if (formals is nil) and (vals is nil):
                ...
                if (formals is nil) or (vals is nil):
                    ...
                    recursor(formals.second, vals.second)
        recursor(formals, vals)
        # END PROBLEM 11
        return child
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        parameters = operands.map(lambda op: scheme_eval(op, env))
        return scheme_apply(self, parameters, env)
        # END PROBLEM 5
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        symbol, value = expressions.first, scheme_eval(expressions.second.first, env)
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    lamdba_instance = LambdaProcedure(formals, expressions.second, env)
    return lamdba_instance
    # END PROBLEM 9
...
def make_let_frame(bindings, env):
    ...
    def make_formals(bindings):
        if bindings is nil:
            return nil
        pair = bindings.first
        check_form(pair, 2, 2)
        return Pair(pair.first, make_formals(bindings.second))

    def make_vals(bindings):
        if bindings is nil:
            return nil
        pair = bindings.first
        check_form(pair, 2, 2)
        return Pair(scheme_eval(pair.second.first, env), make_vals(bindings.second))

    formals, vals = make_formals(bindings), make_vals(bindings)
...
def do_mu_form(expressions, env):
    ...
    check_formals(formals)
    # BEGIN PROBLEM 16
    mu_instance = MuProcedure(formals, expressions.second)
    return mu_instance
    # END PROBLEM 16
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN Extra Credit
            result = scheme_apply(scheme_eval(first, env), rest.map(lambda f: scheme_eval(f, env)), env)
            # END Extra Credit
    return result
